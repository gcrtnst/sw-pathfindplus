...
function buildPathfinder()
    ...
    function pf:_init()
        ...
        self:_initTerrainTable()
        self:_initTileList()
        self:_initNodeList()
    end

    function pf:_initTerrainTable()
        self._terrain_tbl = {
        ...
        for x = self._world_x1, self._world_x2, self._tile_size*2 do
            for z = self._world_z1, self._world_z2, self._tile_size*2 do
                ...
                table.insert(self._tile_list, {
                    x = x,
                    z = z,
                    terrain = self._terrain_tbl[tile_data['name']] or {},
                })
                ::continue::
            end
        end
    ...
    function pf:_initNodeList()
        ...
        for x = self._world_x1 - self._tile_size, self._world_x2 + self._tile_size, self._tile_size*2 do
            for z = self._world_z1 - self._tile_size, self._world_z2 + self._tile_size, self._tile_size*2 do
                local ok = true
                for _, dir in pairs({1, 3, 7, 9}) do
                    local tile_x = x + self._tile_size*((dir - 1)%3 - 1)
                    local tile_z = z + self._tile_size*((dir - 1)//3 - 1)
                    local tile_idx = (tile_tbl[tile_x] or {})[tile_z]
                    if tile_idx ~= nil and self._tile_list[tile_idx].terrain[10 - dir] then
                        ok = false
                        break
                    end
                end
        ...
        for _, this_node in pairs(self._node_list) do
            local dir_tbl = {}
            for _, dir in pairs({1, 3, 7, 9}) do
                local tile_x = this_node.x + self._tile_size*((dir - 1)%3 - 1)
                local tile_z = this_node.z + self._tile_size*((dir - 1)//3 - 1)
                if (tile_tbl[tile_x] or {})[tile_z] == nil then
                    dir_tbl[dir] = true
                end
            end
            for _, dir in pairs({2, 8}) do
                local e_tile_x = this_node.x + self._tile_size
                local e_tile_z = this_node.z + self._tile_size*((dir - 1)//3 - 1)
                ...
                local w_tile_z = this_node.z + self._tile_size*((dir - 1)//3 - 1)
                local w_tile_idx = (tile_tbl[w_tile_x] or {})[w_tile_z]
                local w_tile = w_tile_idx ~= nil and self._tile_list[w_tile_idx] or nil
                if (e_tile == nil or not e_tile.terrain[4]) and (w_tile == nil or not w_tile.terrain[6]) then
                    dir_tbl[dir] = true
                end
            end
            for _, dir in pairs({4, 6}) do
                local n_tile_x = this_node.x + self._tile_size*((dir - 1)%3 - 1)
                ...
                local s_tile_x = this_node.x + self._tile_size*((dir - 1)%3 - 1)
                ...
                if (n_tile == nil or not n_tile.terrain[2]) and (s_tile == nil or not s_tile.terrain[8]) then
                    dir_tbl[dir] = true
                end
            end

            local edge_tbl = {}
            for dir, _ in pairs(dir_tbl) do
                local next_node_x = this_node.x + (self._tile_size*2)*((dir - 1)%3 - 1)
                local next_node_z = this_node.z + (self._tile_size*2)*((dir - 1)//3 - 1)
